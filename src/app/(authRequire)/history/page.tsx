...
import { Search, Trash2, Image as ImageIcon, Video, Download } from 'lucide-react';
import { useAssetHistory } from '@/lib/hooks/useAssetHistory';
import { AssetHistory, AssetType } from '@/types/asset-history';

export default function HistoryPage() {
  const { listHistory, deleteHistory, searchHistory, getStats, loading, error } = useAssetHistory();

  const [history, setHistory] = useState<AssetHistory[]>([]);
  const [filter, setFilter] = useState<AssetType | 'all'>('all');
  const [searchQuery, setSearchQuery] = useState('');
  const [stats, setStats] = useState<{ totalImages: number; totalVideos: number; total: number } | null>(null);
  ...
  const loadHistory = async () => {
    ...
    const result = await listHistory({ ...params, limit: PAGE_SIZE, offset: 0 });
  ...
  const handleLoadMore = async () => {
    ...
    const params = filter === 'all' ? {} : { asset_type: filter };
    const result = await listHistory({ ...params, limit: PAGE_SIZE, offset: nextPage * PAGE_SIZE });
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        {!loading && history.length === 0 && (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm">이미지나 비디오를 생성하면 여기에 표시됩니다</p>
          </div>
        )}
